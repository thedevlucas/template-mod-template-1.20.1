...
public class ObjectiveComplete implements IDialogueWindow {
    private final MinecraftClient client;
    private final long startTime;
    private final Identifier success_bar = new Identifier(TemplateMod.MOD_ID, "textures/gui/objective_status/success_mission_bar.png");
    private float progress = 0.0F;

    public ObjectiveComplete(MinecraftClient client) {
        this.client = client;
    ...
    public void render(DrawContext context, float tickDelta) {
        ...
        int descriptionX = x + width - client.textRenderer.getWidth("ᴍɪssɪᴏɴ ᴄᴏᴍᴘʟᴇᴛᴇᴅ");

        matrix.push();
        matrix.translate(currentX - x - width - 10, 0, 0);

        context.drawTexture(success_bar, x, 50, 0, 0, width, 6, width, 6);
        context.drawTextWithShadow(client.textRenderer, "ᴍɪssɪᴏɴ ᴄᴏᴍᴘʟᴇᴛᴇᴅ", descriptionX, 35, 0xFFFFFFFF);
